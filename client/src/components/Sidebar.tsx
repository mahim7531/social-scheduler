...
import { CalendarIcon, LayoutDashboardIcon, LogOutIcon, UserIcon, Wand2Icon } from 'lucide-react'
...
const Sidebar = ({isOpen,setIsOpen} : {isOpen:boolean,setIsOpen:(val:boolean) => void}) => {
  ...
  const location=useLocation();
    const navItems=[
        ...
        {name:"Scheduler", icon: CalendarIcon,path:"/scheduler"},
          ...
          <nav className="flex-1 px-3 space-y-1 ">
            {
              navItems.map((item)=>{
                ...
                return (
                  ...
                  className={`flex items-center gap-3 py-2.5 rounded text-sm transition-all duration-150 border ${isActive ? "border-red-500 bg-red-100 text-red-700" : "border-transparent hover:bg-slate-100 text-slate-600" }`} >
                    <item.icon className={`size-4.5 shrink-0 ${isActive ? "bg-red-50 text-red-600 border-red-100" : "text-slate-500 hover:bg-slate-50 border-transparent hover:text-slate-700"}`}/>
                    {
                      item.name
                    }
           ...
           <div className="p-4 border-t border-slate-100 ">
            <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50 transition-colors">
             ...
             <button onClick={logout} className="mt-1 flex items-center gap-2 px-3 py-2 w-full rounded text-sm text-slate-500 hover:bg-red-50  hover:text-red-50 hover:text-red-500 transition-all duration-150">
